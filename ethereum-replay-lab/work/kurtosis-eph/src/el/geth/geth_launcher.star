...
def get_config(
    ...
    cmd = [
        ...
        "{0}".format(
            # v1.14.3 has no --override.genesis (that flag was added for
            # Osaka/Prague timestamp overrides in later Geth). Instead we
            # init the datadir with the EL genesis, then start geth.
            # See ENTRYPOINT_ARGS handling below.
            ""
            if False
            else ""
        ),
    ...
    # v1.14.3 uses geth init (no --override.genesis). Prepend an init of
    # the datadir with the EL genesis so the chain starts from the
    # generated genesis.json. genesis is mounted at
    # GENESIS_CONFIG_MOUNT_PATH_ON_CONTAINER.
    cmd_str = (
        "geth init --datadir={0} {1}/genesis.json && ".format(
